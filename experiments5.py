...
def train_net_fedgan(net_id, net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, device="cpu"):
    ...
    G_output_list = None
    sample_idx = 0  # 用于记录 feature_map 的样本位置
    ...
    for epoch in range(epochs):
        ...
        for tmp in train_dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                out = net(x)  # 假设返回的 feature_map 是你想记录的

                # 仅在最后一个 epoch 时记录 G_output
                if epoch == epochs - 1:
                    G_output_list[sample_idx:sample_idx + len(x)] = feature_map.detach()
                    sample_idx += len(x)
...
#     features: 生成器输出的特征向量，形状为 [batch_size, channel, height, width]
#     """
#     batch_size, channel, height, width = features.shape  # 现在features有四个维度
#
#     entropy_values = []
#
#     # 将每个通道展平，并计算该通道的熵
#     for c in range(channel):  # 遍历每个通道
#         feature_c = features[:, c, :, :].detach().cpu().numpy().flatten()  # 展平每个通道
#
#         # 使用KDE估计概率密度
#         kde = KernelDensity(kernel='gaussian', bandwidth=0.1).fit(feature_c.reshape(-1, 1))
#         log_prob = kde.score_samples(feature_c.reshape(-1, 1))
#
#         # 计算熵：H = - sum(p(x) * log(p(x)))
#         prob = np.exp(log_prob)  # 从对数概率恢复概率分布
#         entropy = -np.sum(prob * log_prob) / batch_size
#         entropy_values.append(entropy)
#
#     return np.mean(entropy_values)

# def calculate_entropy_kde(features, batch_size=1000, bandwidth=0.1):
#     """
#     使用核密度估计（KDE）计算整个 batch 数据的熵，分批处理数据
#     features: 生成器输出的特征向量，形状为 [batch_size, channel, height, width]
#     batch_size: 分批处理的大小
#     bandwidth: KDE带宽参数，用于控制估计的平滑程度
#     """
#     # 获取输入数据的形状
#     total_samples, channel, height, width = features.shape
#     # 将每个样本的 [channel, height, width] 展平为一维向量
#     flattened_features = features.view(total_samples, -1).detach().cpu().numpy()
#
#     # 初始化熵值列表
#     entropy_values = []
#
...
#         batch_features = flattened_features[start_idx:end_idx]
#
#         # 使用KDE估计当前批次的概率密度
#         kde = KernelDensity(kernel='gaussian', bandwidth=bandwidth).fit(batch_features)
#         log_prob = kde.score_samples(batch_features)
#         # print(log_prob) # 1409.94632195
#         # 对log_prob进行裁剪，防止溢出
#         log_prob = np.clip(log_prob, -700, 700)  # 限制log_prob的范围
#
#         # 计算熵：H = - sum(p(x) * log(p(x)))
#         # 直接用log_prob来计算，避免乘法引发的溢出
#         # print(log_prob) # 700
#         entropy = -np.sum(log_prob) / batch_features.shape[0]
#         entropy_values.append(entropy)
#
#     # 返回所有批次的熵的平均值
#     return np.mean(entropy_values)
