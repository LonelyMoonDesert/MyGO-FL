...
def extract_selective_accuracy_from_logs(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.log'):
            ...
            with open(os.path.join(directory, filename), 'r') as file:
                ...
                # If the method is 'fedgan', only keep even-indexed items
                if method_name == 'fedgan':
                    accuracies = [acc for idx, acc in enumerate(accuracies) if idx % 2 != 0]
    ...
    df.to_csv(os.path.join(directory, 'global_model_test_accuracies_10.csv'), index=False)

# Example usage
directory_path = '../logs/vgg9-cifar10/noniid-labeldir(beta0.5)'  # Change this to your log files directory
extract_selective_accuracy_from_logs(directory_path)
